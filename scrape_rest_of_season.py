...
with app.app_context():
    ...
    game_year = last_game.date.year
    game_month = last_game.date.month
    ...
    games_on_last_date = [game for game in Game.query.all() if game.date.date()==last_game.date.date()]

    last_game_date = last_game.date.date()
    ...
    for game in rows:
        ...
        game_date = datetime.datetime.strptime(game.select('th')[0].text,'%Y-%m-%d').date()
